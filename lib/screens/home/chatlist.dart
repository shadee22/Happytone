// ignore_for_file: prefer_const_constructors, avoid_print, use_key_in_widget_constructors, unused_local_variable, must_be_immutable, prefer_const_literals_to_create_immutables, prefer_is_empty, unnecessary_this
...
class _ChatlistState extends State<Chatlist> {
  ...
  datas() async {
    final data = db.getUserData('shadeer');
    print(data);
  }

  Widget searchList() {
    return ListView.builder(itemBuilder: (x, i) {
      return SearchTile(
        username: allusers?.docs[i].get('name'),
        useremail: allusers?.docs[i].get('email'),
      );
    });
  }
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          ...
          return ListView.builder(
              ...
              itemBuilder: (context, index) {
                //VARIABLES
                final username = snapshot.data.docs[index].get("name").toString().inCaps;
                final useremail = snapshot.data.docs[index].get("email").toString().inCaps;
                final userpassword = snapshot.data.docs[index].get("password").toString().inCaps;
